(function () {
  ...
  function sectionBounds(entry, heading) {
    const start = entry.search(new RegExp(`\\n\\n## ${heading}\\b`));
    if (start < 0) {
      return null;
    }

    const contentStart = start + 2;
    const afterHeading = entry.slice(contentStart + `## ${heading}`.length);
    const nextHeading = afterHeading.search(/\n\n## /);
    const end = nextHeading >= 0
      ? contentStart + `## ${heading}`.length + nextHeading
      : entry.length;
    return { start, contentStart, end };
  }

  function insertSection(entry, section, beforeHeadings) {
    const insertBefore = entry.search(new RegExp(`\\n\\n## (${beforeHeadings})\\b`));
    const insertAt = insertBefore >= 0 ? insertBefore : entry.length;
    const prefix = entry.slice(0, insertAt).trimEnd();
    const suffix = entry.slice(insertAt);
    return `${prefix}\n\n${section}${suffix}`;
  ...
  function missingEmbeds(section, paths, prefix = "") {
    return paths
      .filter(Boolean)
      .filter((path) => !section.includes(assetEmbed(path)))
      .map((path) => `${prefix}${assetEmbed(path)}`);
  }

  function ensureMediaSection(entry, savedAssets = {}) {
    const embeds = (savedAssets.images || []).filter(Boolean);
    if (!embeds.length) {
      return entry;
    }

    const bounds = sectionBounds(entry, "Media");
    if (!bounds) {
      return insertSection(
        entry,
        ["## Media", "", ...embeds.map(assetEmbed)].join("\n"),
        "Video|Quote Tweet|In Reply To|Article"
      );
    }

    const section = entry.slice(bounds.contentStart, bounds.end);
    const missing = missingEmbeds(section, embeds);
    if (!missing.length) {
      return entry;
    }
    return `${entry.slice(0, bounds.end).trimEnd()}\n${missing.join("\n")}${entry.slice(bounds.end)}`;
  }
  ...
  function ensureQuoteImages(entry, savedAssets = {}) {
    const embeds = (savedAssets.quoteImages || []).filter(Boolean);
    if (!embeds.length) {
      return entry;
    }

    const bounds = sectionBounds(entry, "Quote Tweet");
    if (!bounds) {
      return entry;
    }
  ...
  function replaceMonthlyEntryText(existingMarkdown, tweet, articleId = "", savedAssets = {}) {
    ...
    const mediaEntry = ensureMediaSection(textEntry, savedAssets);
    const videoEntry = replaceVideoSection(mediaEntry, tweet, savedAssets);
    const quoteImageEntry = ensureQuoteImages(videoEntry, savedAssets);
    const articleEntry = replaceArticleSection(quoteImageEntry, tweet, articleId);
